# Figma Test Case Discovery: Sophi - Developers Hand Off (Node 8062-9900)

**Source:** [Sophi - Developers Hand Off](https://www.figma.com/design/yFFgIDlTbDcQAbICJyFEuu/Sophi---Developers-Hand-Off?node-id=8062-9900&t=kqucwgizz0RE5adN-4)  
**Analysis Date:** 2026-02-17  
**Tool:** figma-test-case-discovery agent  
**Requested node:** `8062:9900`

---

## ⚠️ Note on scope

**Live Figma extraction for node `8062:9900` was not run** because the Figma API requires `FIGMA_TOKEN`, which was not set in this run. This report is based on the same file’s known structure and a prior full-file analysis (same design system, flows, and components).

**To get node-specific coverage** (frames, components, and interactions for the exact node 8062-9900):

1. Set your Figma Personal Access Token (Figma → Settings → Personal access tokens):
   ```bash
   export FIGMA_TOKEN="figd_..."
   ```
2. If the file is password-protected:
   ```bash
   export FIGMA_PASSWORD="your-password"
   ```
3. Run the discovery fetch:
   ```bash
   ./scripts/run-figma-discovery.sh
   ```
4. Then ask the agent: *"Generate test case discovery from figma-data/node-8062-9900-full.json"* to get a report derived from that node’s structure and interactions.
...
**Assumptions:** No live extraction for node `8062:9900`. Test cases below apply to the Sophi file as a whole; when the frame at node 8062-9900 is identified (e.g., a specific screen or flow), these cases should be mapped to that frame as applicable.
...
*End of Figma Test Case Discovery (node 8062-9900)*
